...
void PLC_MASTER::PushMQTT(String Payload, String Topic){
  #ifdef MQTT_USE
  // if(mqttConnected){mqttPLC.mqttPublish(Payload, Topic);}
  #endif//MQTT
  }
...
if(UpdateFirmware==false){
  ...
  if(connectWebSocket == 0){
    ...
    if (millis() - previousMillis >= 100 ) {digitalWrite(LED_STATUS, !digitalRead(LED_STATUS));previousMillis = millis();Time_loop();}
  }
  if(connectWebSocket == 1){
    static unsigned long previousMillis = 0;
    if (millis() - previousMillis >= 300 ) {digitalWrite(LED_STATUS, !digitalRead(LED_STATUS));previousMillis = millis();}Time_loop();
  }
/////
#ifdef MQTT_USE
  #ifdef MQTTSSL
...
#ifdef MQTT_USE
mqttConnected = PLC_Cofig.MQTTConnected;//mqttPLC.connect_state();
...
static int EnvKey[17] = {1249,1233,1234,1235,1236,1237,1238,1239,1240,1241,1242,1243,1244,1245,1246,1247,1248};
String ConnID = "30f1c692-c183-4b35-8466-167ec026c61c";
//VOM
String Topic_iMag = "/connectivity/in";
    ...
    timeClient.update();
      ...
      #endif//VOM_PayloadV0
      #ifdef VOM_PayloadV1
// {"connId":"8e991a84-351c-4d7a-9d37-110da5546aea","data":[{"key":"846","value":"17042","updateAt":"1725635214"},{"key":"847","value":"4003","updateAt":"1725635214"}],"exeAt":"2024-9-6 15:6:54"}
      String TimeAt = String(Getyear) + "-" + String(Getmonth) + "-" + String(Getday) + " " + String(Gethour) + ":" + String(Getmin) + ":" + String(Getsec);

      root["connId"] = ConnID;
      for(byte c = 0 ; c < 17 ; c++){
        root["data"][c]["key"] = EnvKey[c];
        root["data"][c]["value"] = FRegister[ValuePosition[c]];
        root["data"][c]["updateAt"] = (GetEpoch);
      }
      root["exeAt"] = TimeAt;
      LOGLN("Time: " + TimeAt);
      serializeJson(root, payload);
      ...
      // LOGLN("MQTT connected: " + String(mqttConnected));
      PLC_MASTER_PROG.PushMQTT(payload, "/connectivity/in");
      MQTTPush(Topic_iMag, payload);
      // PLC_MASTER_PROG.PushMQTT("Done", "/datalogger/shiratechPoE/stations/Line1/monitor");
      #endif//VOM_PayloadV1
#endif//
#endif//#ifdef VOM
